...
declare namespace React {
    ...
    type WithCapture<T> = {
        [Key in keyof T as Key extends string ? Key | `${Key}Capture` : never]?: T[Key]
    }


    interface DOMAttributes<T> extends WithCapture<DomEventsWithCapture<T>> {
        children?: ReactNode | undefined;
    ...
    type AutoFillContactField =
        | "email"
        | "tel"
        | `tel-${"area-code" | "country-code" | "extension" | "local" | "local-prefix" | "local-suffix" | "national"}`;
    ...
    type AutoFillNormalField =
        ...
        | `address-line${1 | 2 | 3}`
        | `bday-${"day" | "month" | "year"}`
        | `cc-${"csc" | "exp" | "exp-month" | "exp-year" | "family-name" | "given-name" | "name" | "number" | "type"}`
        | "country"
        | "country-name"
        | "current-password"
        | "family-name"
